...
class Noise {
  ...
  perlin2(x, y) {
    ...
    const n11 = this.gradP[X + 1 + this.perm[Y + 1]].dot2(nextX - 1, nextY - 1);
    const u = this.fade(nextX);

    return this.lerp(this.lerp(n00, n10, u), this.lerp(n01, n11, u), this.fade(nextY));
  }
}
...
export default function Waves({
  ...
  useEffect(() => {
    ...
    function setLines() {
      ...
      const totalLines = Math.ceil(oWidth / nextXGap);
      const totalPoints = Math.ceil(oHeight / nextYGap);
    ...
    function drawLines() {
      ...
      linesRef.current.forEach((points) => {
        ...
        points.forEach((point, index) => {
          ...
          currentContext.lineTo(p1.x, p1.y);
          if (isLast) currentContext.moveTo(p2.x, p2.y);
        });
      });
